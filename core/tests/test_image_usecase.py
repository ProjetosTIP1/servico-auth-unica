...
class TestImageUsecase:
    ...
    async def test_upsert_user_profile_picture_success(
        ...
        # Act
        await image_usecase.upsert_user_profile_picture(
            mock_txn, mock_upload_file, user_id
        )
    ...
    async def test_upsert_from_bytes_success(
        self, mock_makedirs, mock_open, mock_uuid, mock_guess, image_usecase, mock_txn
    ):
        # Arrange
        user_id = 1
    ...
    async def test_upsert_user_profile_picture_db_error_triggers_rollback(
        ...
        # Act & Assert
        with pytest.raises(
            Exception, match="Error in service layer while upserting image"
        ):
            await image_usecase.upsert_user_profile_picture(
                mock_txn, mock_upload_file, 1
            )
